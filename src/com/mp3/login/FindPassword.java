...
public class FindPassword extends JPanel{
	...
	public FindPassword(FindIdPassword fip) {
		...
		l_id.setPreferredSize(d);
		l_id.setFont(font);
		l_id.setBackground(Color.BLACK);
		l_id.setForeground(Color.GREEN);
		t_id.setPreferredSize(ld);
		t_id.setFont(font);
	...
	public void findPW() {
		...
		String sql = "select password from memberlist where id = '" + id + "' and name = '" + name + "' and birth = '" + birth +"'";
		
		try {
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if(rs.next()) {
				JOptionPane.showMessageDialog(fip.mp3_Main, "찾으신 password는 " + rs.getString("password") + " 입니다.");
			}else {
